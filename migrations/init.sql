...
CREATE OR REPLACE FUNCTION orbital()
RETURNS TRIGGER LANGUAGE PLPGSQL AS 
$$
BEGIN
...
CREATE OR REPLACE FUNCTION engine()
RETURNS TRIGGER LANGUAGE PLPGSQL AS 
$$
BEGIN
    DELETE FROM Plans;
...
CREATE OR REPLACE TRIGGER parameter_trigger AFTER DELETE ON Parameters
FOR EACH STATEMENT EXECUTE PROCEDURE parameters();


CREATE OR REPLACE TRIGGER tle_trigger AFTER INSERT OR UPDATE OR DELETE ON TLEs
FOR EACH STATEMENT EXECUTE PROCEDURE orbital();


CREATE OR REPLACE TRIGGER pass_trigger AFTER INSERT OR UPDATE OR DELETE ON Passes
FOR EACH STATEMENT EXECUTE PROCEDURE engine();
